...
int main() {
	...
	Texture f;
	f.loadFromFile("E:/Paint/fon.png");
	Sprite fon(f);

	Texture c;
	c.loadFromFile("E:/Paint/crnol.png");
	Sprite vibor[2];
	for (int i = 0; i < 2; i++) {
	...
	Texture l;
	l.loadFromFile("E:/Paint/line.png");
	Sprite line(l);
	...
	while (window.isOpen())
	{
			...
			if (event.type == Event::MouseButtonPressed)
				if (event.key.code == Mouse::Left) {
					if (vib == 0) {
						...
						for (int i = 0; i < 9; i++)
							if (player.sprite[i].getGlobalBounds().contains(pos.x, pos.y) && !FreeCell[i]) {
								player.tik[i] = true;
								FreeCell[i] = true;
								sv++;
		...
		if (hotbot && !win && sv < 9) {
			...
			if (!dumbot) {
				...
				for (int i = 0; i < 3; i++) {
					if (player.tik[i] && player.tik[3 + i] && !FreeCell[6 + i]) {
						botstav = 6 + i;
						cdat = false;
					}
					if (player.tik[6 + i] && player.tik[3 + i] && !FreeCell[i]) {
						botstav = i;
						cdat = false;
					}
					if (player.tik[3 * i] && player.tik[1 + 3 * i] && !FreeCell[2 + 3 * i]) {
						botstav = 2 + 3 * i;
						cdat = false;
					}
					if (player.tik[2 + 3 * i] && player.tik[1 + 3 * i] && !FreeCell[3 * i]) {
						botstav = 3 * i;
						cdat = false;
					}
				}
				if (player.tik[0] && player.tik[4] && !FreeCell[8]) {
					botstav = 8;
					cdat = false;
				}
				if (player.tik[8] && player.tik[4] && !FreeCell[0]) {
					botstav = 0;
					cdat = false;
				}
				if (player.tik[2] && player.tik[4] && !FreeCell[6]) {
					botstav = 6;
					cdat = false;
				}
				if (player.tik[6] && player.tik[4] && !FreeCell[2]) {
					botstav = 2;
					cdat = false;
				}
				if (player.tik[2] && player.tik[6] && !FreeCell[4]) {
					botstav = 4;
					cdat = false;
				}
